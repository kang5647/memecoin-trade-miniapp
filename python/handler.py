...
async def get_token_info(contract_address: str, chain: str='base') -> dict:
    """
...
async def tokeninfo(update: Update, context: CallbackContext) -> None:
    if not context.args:
        await update.message.reply_text("Please provide a chain and contract address. Usage: /token <contract_address>")
        return
    ...
    try:
        ...
        response = (f"🪙 Token Information for {token_info['name']} ({token_info['symbol']}):\n\n"
                    f"📊 Basic Info:\n"
        ...
        await update.message.reply_text("Sorry, I couldn't fetch the token information. Please try again later.")
